...
// `waiting` — it never interrupts a running session with a confirm dialog.
// The next explicit check picks that waiting worker up and prompts for it.
//
// While the device is being polled no check runs at all (see
// setUpdateChecksSuspended): the only thing an update can offer mid-session is
// a reload that would drop the connection and stop the polling.
...
// would drop the polling mid-session, so there is nothing useful to find out.
...
const promptAndActivate = async (worker: ServiceWorker) => {
  ...
  const shouldActivate = window.confirm(
    ...
    'Warning: Reloading will stop any active polling.'
  );
  if (shouldActivate) {
