// Service Worker for DigitShowWebview PWA
...
// with a sibling app on the same origin would put both apps' shells in one
// namespace — and the activate handler below, which deletes every cache
// carrying this prefix except the current one, would take the other app's
// precache with it.
const CACHE_PREFIX = 'digit-show-webview-';
const CACHE_NAME = `${CACHE_PREFIX}${CACHE_VERSION}`;
const BASE_PATH = '/digit_show_webview/';
...
// Pre-cache: every build asset (hashed JS/CSS bundles, index.html, manifest,
// icon...) is cached during install so the app shell works fully offline after
// the first successful online visit. The list is injected at build time by the
// `precache-manifest` plugin in vite.config.ts (it replaces the empty array
// below). It stays empty for unbuilt `vite dev`, where the Service Worker is
// inactive anyway (different base path).
const PRECACHE_MANIFEST = [];
const PRECACHE_URLS = [
  // The start_url (`/digit_show_webview/`) resolves to index.html but is a
